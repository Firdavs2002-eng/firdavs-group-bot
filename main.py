...
# --- ALOHIDA TUGMALAR (NINIOSHOP STYLE) ---
def get_main_menu(user_id, lang):
    ts = int(time.time()) # Anti-cache tizimi (Dizayn qotmasligi uchun)
    
    # URL manzillariga e'tibor bering, app=shop va app=game deb aniq ajratilgan
...
async def process_cat_name(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(f"📸 <b>{cat_name}</b> uchun rasm yuboring (Iloji boricha kvadrat rasm):", parse_mode="HTML")
...
async def process_cat_photo(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await wait_msg.edit_text(f"✅ <b>{cat_name}</b> rasmi muvaffaqiyatli o'zgardi.", parse_mode="HTML")
    except Exception:
        await wait_msg.edit_text("❌ Xatolik yuz berdi. Boshqa rasm sinab ko'ring.")
    await state.clear()
...
async def api_send_code(request):
    ...
    if not uid or not phone: return web.json_response({"error": "Parametrlar yetishmayapti"}, status=400, headers=set_cors())
    ...
    try:
        ...
        return web.json_response({"error": "Bot foydalanuvchiga yoza olmadi"}, status=500, headers=set_cors())

async def main():
    init_db()
    app = web.Application()
    app.router.add_get('/', lambda r: web.Response(text="Firdavs Group Server is Running!"))
